...
import { forceCodec, modifyOpusDtxParam } from '../../src/js/utils';
...
describe('forceCodec', () => {
    var expectedSdp = "v=0\r\n" +
        ...
        "a=ssrc:2534193841 mslabel:idv6kIIdFJ9x3alklN0n3uNGmI8xgFecIJkt\r\n" +
        "a=ssrc:2534193841 label:d26be488-17a9-4f63-85b4-bdcf9754bb9b\r\n";
    it('Removes unmatching codecs', () => {
        chai.expect(forceCodec(inputSdp, 'audio', 'opus')).to.eq(expectedSdp);
        chai.expect(forceCodec(inputSdp, 'audio', 'OPUS')).to.eq(expectedSdp);
    });

    it('Removes nothing without matching media type', () => {
        chai.expect(forceCodec(inputSdp, 'video', 'opus')).to.eq(inputSdp);
    });
    ...
    it('Removes unmatching codecs', () => {
        chai.expect(forceCodec(inputSdp, 'audio', 'PCMU')).to.eq(expectedPcmuSdp);
    });
});

describe('modifyOpusDtxParam', () => {
    ...
    it('Adds usedtx=1 if enableDtx is true', () => {
        chai.expect(modifyOpusDtxParam(inputSdp, true)).to.eq(expectedSdpWithDtx);
    });

    it('Adds usedtx=0 if enableDtx is false', () => {
        chai.expect(modifyOpusDtxParam(inputSdp, false)).to.eq(expectedSdpWithoutDtx);
    });

    it('Adds usedtx=0 if enableDtx is not set', () => {
        chai.expect(modifyOpusDtxParam(inputSdp)).to.eq(expectedSdpWithoutDtx);
    });
});