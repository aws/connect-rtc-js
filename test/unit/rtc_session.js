...
describe('RTC session', () => {
    ...
    describe('getStats', () => {
        ...
        beforeEach(() => {
            const dependencyModule = require("../../src/js/rtp-stats");
            sinon.stub(dependencyModule, "extractMediaStatsFromStats").returns(null);
            session = new RtcSession('wss://amazon-connect-rtc-server.amazonaws.com/', [], 'contactToken', console);
            session._legacyStatsReportSupport = true;
            var rawStats = {
                result: () => {return null}
            }
            session._pc = {
                signalingState: 'stable',
                getStats:  (callback) => {setTimeout(function() {
                    callback(rawStats);
                  }, 1)}
            };
            var audioTrack = {
        ...
        it('for legacy stats, when digested stats is empty, error is handled', () => {
            var errorCatched = false;
            session.getRemoteAudioStats().then(function () {
            }, function (error) {
                console.log(error);
                errorCatched = true;
            }).finally(() => {
                chai.expect(errorCatched).to.be.true;
            });
            clock.tick(5);
        });
    });
