...
import { extractMediaStatsFromStats } from '../../src/rtp-stats';
...
describe('extractMediaStatsFromStats', () => {
    describe('standardized get stats', () => {
        ...
        it('is able to get audio_output stats', () => {
            ...
            chai.expect(digestedStats.jitterMilliseconds).to.eq(6);
            chai.expect(digestedStats.echoReturnLoss).to.eq(-28.169708251953125);
            chai.expect(digestedStats.echoReturnLossEnhancement).to.eq(0.17551203072071075);
        ...
        it('is able to get audio_input stats', () => {
            ...
            chai.expect(digestedStats.jitterMilliseconds).to.eq(8);
            // jbMilliseconds is cumulative jitterBufferDelay in ms (28723.2 * 1000 = 28723200)
            chai.expect(digestedStats.jbMilliseconds).to.eq(28723200);
            chai.expect(digestedStats.jitterBufferEmittedCount).to.eq(419520);
            chai.expect(digestedStats.streamType).to.eq('audio_input');
            chai.expect(digestedStats.concealmentEvents).to.eq(10);
        });
    });
});
