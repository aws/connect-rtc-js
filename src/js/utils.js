...
const LOCAL_ADDRESS_REGEX = /(?:(?:192\.)(?:(?:168\.)(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?\.)(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)))|(?:(?:10\.)(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){2}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?))|(?:(?:172\.)(?:(?:1[6-9]|2[0-9]|3[0-1])\.)(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.)(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?))/gm;
...
export function checkMediaEndpointConnection(mediaEndpoint) {
    const username = "Test";
 ...
 * @returns {boolean}
 */
export const isTypRelay = (payload) => {
 ...
 * @returns {boolean}
 */
export const isTypHost = (payload) => {
 ...
 * @returns {boolean}
 */
export const isLocalAddress = (address) => {
...
export const getSdpCandidate = (sdp) => {
    // let s = sdpTransform.parse(sdp);
    var sdpOptions = new SdpOptions();
    var s = transformSdp(sdp, sdpOptions);
    console.log("Reached here : {}", s);

    for (let m=0; m<s.media.length; m++) {
        let media = s.media[m];

        for (let c=0; c<media.candidates.length; c++) {
            let candidate = media.candidates[c];
